...
def create_random_boid(width, height):
    return Boid(
        ...
        bounds=[width, height],
        velocity=[random.uniform(-50.0, 50.0), random.uniform(-50.0, 50.0)]) # removed color

def get_window_config():
    #platform = pyglet.window.get_platform()
    display = pyglet.canvas.get_display() #platform.get_default_display()
...
def run():
    ...
    mouse_location = (0, 0)
    window = pyglet.window.Window(1500, 900,
        fullscreen=False,
        resizable=True,

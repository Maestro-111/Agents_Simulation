...
class Boid:
    def __init__(self,
        ...
        self.size = size
        self.color = random.choice([[1.0, 0, 0],[0, 1.0, 0]])
        self.change_vectors = []
    ...
    def average_position(self, nearby_boids):
        # take the average position of all nearby boids-bda450, and move the boid towards that point
    ...
    def average_velocity(self, nearby_boids):
        # take the average velocity of all nearby boids-bda450
    ...
    def update(self, dt, all_boids, attractors, obstacles):
        ...
        enemies = [i for i in nearby_boids if i.color != self.color]
        allies = [i for i in nearby_boids if i.color == self.color]

        # update the boid's direction based on several behavioural rules
        cohesion_vector = self.average_position(allies)
        alignment_vector = self.average_velocity(allies)
        enemies_pos = list(np.array(self.average_position(enemies))*-1)
        ...
        self.change_vectors = [
            ...
            (_OBSTACLE_AVOIDANCE_FACTOR, enemies_pos)]
